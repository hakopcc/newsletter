...
namespace ArcaSolutions\ModStoresBundle\Plugins\ArticleAssociationListing\Command;
...
class EdirectoryModstoresInstallCommand extends AbstractEdirectoryModstoresInstallCommand
{
    ...
    public function commandWorkflow()
    {
        ...
        $this->overwriteIndexCreation(['mappings', 'article', 'properties', 'listingId', 'type'], 'integer');
        ...
        $this->simpleLog('[-] Copy listing widget placeholder');
        $this->copyListingWidgetPlaceholders($this->bundle->getName());

        $this->simpleLog('[-] Copy Stub files');
        $this->copyStub($this->bundle->getName(), 'article-tab.php', 'web/sitemgr/content/listing/article.php');
    }
    ...
    public function afterCommandWorkflow()
    {
        ...
        $this->appendToData('afterCommand', [
            'fixture:domain' => [
                'fixtures' => [
                    'src/ArcaSolutions/ModStoresBundle/Plugins/ArticleAssociationListing/DataFixtures/ORM/Common',
                ],
            ],
        ]);
    }
