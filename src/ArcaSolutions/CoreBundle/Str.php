...
class Str
{
    ...
    public static function sub($str, $start, $length = null, $encoding = null)
    {
        ...
        $length = is_null($length) ? (function_exists('mb_substr') ? mb_strlen($str,
                $encoding) : strlen($str)) - $start : $length;
    ...
    /**
     * Returns a set of words without the apostles
     *
     * @author Diego Mosela <diego.mosela@arcasolutions.com>
     * @since 11.3.00
     * @todo Function inserted to keep the pattern of the fulltextsearch field in import.
     *       In the future may come to be removed, because this treatment already be done by elasticsearch
     *
     * @param $string
     * @return array
     */
    public static function replaceApostleWords($string)
    {
        ...
        foreach ($words as $word) {
            ...
            /* Adds the word to be returned */
            if (isset($newWord)) {
                $newWords[] = $newWord;
            }
        }

        return array_unique($newWords);
    }
     ...
     */
    public static function htmlEntities($string, $flags = ENT_COMPAT, $charset = EDIR_CHARSET, $double_encode = true) {
